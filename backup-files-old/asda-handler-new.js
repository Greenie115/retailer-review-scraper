...
async function handleAsdaSite(page, siteConfig, maxReviews = 50) {
  ...
  try {
    ...
    // First, handle cookie consent if present - enhanced version
    try {
      // Take a screenshot before handling cookies
      await page.screenshot({ path: `asda-before-cookies-${Date.now()}.png` });

      // Try multiple selectors for the cookie button
      const cookieSelectors = [
        'button[data-auto-id="onetrust-accept-btn-handler"]',
        ...
        try {
          ...
          if (cookieButton) {
            ...
            try {
              ...
              await cookieButton.click({ force: true }).catch(async (e) => {
                log.warning(`Direct cookie click failed: ${e.message}, trying JavaScript click...`);

                // Method 2: JavaScript click
                await page.evaluate(button => button.click(), cookieButton);
              });

              cookieButtonClicked = true;
              log.info('Successfully clicked cookie button');
              await page.waitForTimeout(2000);
              break;
            } catch (clickError) {
              log.warning(`Error clicking cookie button with selector ${selector}: ${clickError.message}`);
            }
          }
        } catch (e) {
          log.warning(`Error finding cookie button with selector ${selector}: ${e.message}`);
        }
      }

      // If we couldn't click the button with selectors, try JavaScript approach
      if (!cookieButtonClicked) {
        log.info('Trying JavaScript approach to handle cookies...');

        const cookieHandled = await page.evaluate(() => {
          // Try to find and click any cookie consent button
          ...
          for (const button of possibleButtons) {
            ...
            if (text.includes('accept') && (text.includes('cookie') || text.includes('consent'))) {
              console.log('Found likely cookie button via text content, clicking...');
              button.click();
              return true;
            }
          }

          // Try to find the OnetrustActiveGroup cookie banner
          const banner = document.getElementById('onetrust-banner-sdk');
          if (banner) {
            const buttons = banner.querySelectorAll('button');
            for (const button of buttons) {
              if (button.textContent.toLowerCase().includes('accept')) {
                console.log('Found cookie button in banner, clicking...');
                button.click();
                return true;
              }
            }
        ...
        if (cookieHandled) {
          log.info('Successfully handled cookies via JavaScript');
          await page.waitForTimeout(2000);
        } else {
          log.warning('Could not find or click cookie consent button with any method');
        }
      }

      // Take a screenshot after handling cookies
      await page.screenshot({ path: `asda-after-cookies-${Date.now()}.png` });
    } catch (cookieError) {
    ...
    if (!tabClicked) {
      ...
      // Use a safer approach that doesn't rely on :contains() pseudo-class
      const clicked = await page.evaluate(() => {
        try {
          console.log('Starting JavaScript approach to find reviews tab');

          // Method 1: Look for tabs with role="tab" that contain "review" text
          const roleTabs = document.querySelectorAll('[role="tab"]');
          console.log(`Found ${roleTabs.length} elements with role="tab"`);

          for (const tab of roleTabs) {
            const text = tab.textContent.toLowerCase();
            if (text.includes('review')) {
              console.log('Found reviews tab with role="tab", clicking...');
              tab.click();
              return true;
            }
          }

          // Method 2: Look for links with href containing "review"
          const reviewLinks = document.querySelectorAll('a[href*="review"]');
          console.log(`Found ${reviewLinks.length} links with href containing "review"`);

          for (const link of reviewLinks) {
            console.log('Found review link, clicking...');
            link.click();
            return true;
          }

          // Method 3: Look for any button or link that might be the reviews tab
          console.log('Trying more aggressive approach to find reviews tab');
          const allElements = document.querySelectorAll('button, a, [role="tab"], li, div[role="button"]');
          console.log(`Found ${allElements.length} potential clickable elements`);

          for (const element of allElements) {
            const text = element.textContent.toLowerCase();
            if (text.includes('review')) {
              console.log('Found element with "review" text, clicking...');
              element.click();
              return true;
            }
          }

          // Method 4: Look for tab containers
          const tabLists = document.querySelectorAll('[role="tablist"]');
          console.log(`Found ${tabLists.length} tab lists`);

          for (const tabList of tabLists) {
            const tabs = tabList.querySelectorAll('[role="tab"], button, a, li');
            console.log(`Found ${tabs.length} tabs in tab list`);

            // If there are multiple tabs, try clicking the second one (often reviews)
            if (tabs.length > 1) {
              console.log('Trying to click the second tab (often reviews)...');
              tabs[1].click();
              return true;
            }
          }

          // Method 5: Look for elements with tab-related classes
          const tabElements = document.querySelectorAll('[class*="tab"], [class*="Tab"]');
          console.log(`Found ${tabElements.length} elements with tab-related classes`);

          for (const element of tabElements) {
            const text = element.textContent.toLowerCase();
            if (text.includes('review')) {
              console.log('Found tab element with "review" text, clicking...');
              element.click();
              return true;
            }
          }
          ...
          console.error('Error in JavaScript approach to find reviews tab:', error);
          return false;
        }
      });
      ...
      if (clicked) {
        ...
        log.warning('Could not find reviews tab with any method, trying to find reviews directly');

        // Try to find reviews directly without clicking the tab
        await page.evaluate(() => {
          // Scroll down to look for reviews
          window.scrollBy(0, 500);

          // Look for review containers directly
          const reviewContainers = document.querySelectorAll(
            'div.pdp-description-reviews__content-cntr, ' +
            '[data-auto-id="review-container"], ' +
          ...
          console.log(`Found ${reviewContainers.length} review containers directly`);

          // If we found review containers, scroll to them
          if (reviewContainers.length > 0) {
            reviewContainers[0].scrollIntoView({ behavior: 'smooth', block: 'center' });
            console.log('Scrolled to review container');
          }
        });
