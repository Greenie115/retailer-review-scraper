...
function updateSainsburysHandler() {
  try {
    console.log('Starting to update the Sainsbury\'s handler with fixed version...');
    ...
    const sainsburysHandlerPath = path.join(__dirname, 'sainsburys-handler-fixed.js');
    ...
    const backupPath = path.join(__dirname, 'review-scraper-crawlee-fixed.backup-sainsburys-fixed.js');
    ...
    console.log(`Updated ${filePath} with the fixed Sainsbury's handler`);
