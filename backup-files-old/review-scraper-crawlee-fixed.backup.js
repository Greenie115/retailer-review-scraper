...
async function handleTescoSite(page, siteConfig, maxReviews) {
  ...
  console.log('DEBUGGING: Using updated Tesco handler');
  ...
  try {
    ...
    const reviews = await page.evaluate(() => {
      ...
      const reviewContainers = document.querySelectorAll('.review, .product-review, .customer-review');
        ...
        try {
          ...
          if (ratingElement) {
            ...
            if (ratingMatch) {
              ...
              console.log(`Extracted Tesco rating: ${rating}`);
            }
          }

          // Extract title
          let title = 'Product Review'; // Default title
          const titleElement = container.querySelector('.review-title, h3, h4');
          if (titleElement) {
            title = titleElement.textContent.trim();
            console.log(`Extracted title: "${title}"`);
          }
          ...
          // Extract date
          let date = 'Unknown date';
          const dateElement = container.querySelector('.review-date, .date');
          if (dateElement) {
            date = dateElement.textContent.trim();
            console.log(`Extracted date: "${date}"`);
          }
...
async function handleMorrisonsSite(page, siteConfig, maxReviews) {
  ...
  console.log('DEBUGGING: Using updated Morrisons handler');
  ...
  try {
    ...
    await page.screenshot({ path: `morrisons-before-next-page-0-${Date.now()}.png` });
    ...
    while (hasMorePages && pageNum <= 10 && allReviews.length < maxReviews) {
      ...
      await page.screenshot({ path: `morrisons-before-next-page-${pageNum}-${Date.now()}.png` });
      ...
      const pageReviews = await page.evaluate(() => {
        ...
        // Find all review containers
        const reviewItems = document.querySelectorAll('[data-test^="review-item-"]');
        ...
        for (const item of reviewItems) {
          try {
            // Extract rating
            let rating = '5'; // Default to 5 stars
            const ratingStars = item.querySelectorAll('svg[href="#svg-star_full"]');
            if (ratingStars && ratingStars.length > 0) {
              rating = ratingStars.length.toString();
              console.log(`Extracted Morrisons rating: ${rating} stars`);
            }
      ...
      // Take a screenshot after extracting reviews on this page
      await page.screenshot({ path: `morrisons-after-next-page-${pageNum}-${Date.now()}.png` });
