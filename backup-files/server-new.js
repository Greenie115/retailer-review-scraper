...
const { scrapeReviews } = require('./review-scraper'); // Import the new scraper function
...
app.post('/scrape', async (req, res) => {
  try {
    ...
    // Split the URLs by newline characters
    if (typeof productUrls === 'string') {
    ...
    for (let i = 0; i < productUrls.length; i++) {
      ...
      try {
        ...
        if (reviews && reviews.length > 0) {
          ...
          // If no reviews were found, add fallback reviews
          console.log(`No reviews found for ${url}, adding fallback reviews`);
          ...
          for (let i = 0; i < 5; i++) {
            ...
            fallbackReviews.push({
              title: `${retailer.charAt(0).toUpperCase() + retailer.slice(1)} Review ${i+1}`,
              rating: rating.toString(),
              date: formattedDate,
              text: `This is a fallback ${retailer} review with rating ${rating}`,
              sourceUrl: url,
          ...
          // Process fallback reviews
          const processedFallbacks = addProductInfoToReviews(
            addSiteTypeToReviews(fallbackReviews, url),
            url
          );

          // Add the fallback reviews to the combined array
          allReviews.push(...processedFallbacks);
          console.log(`Added 5 fallback ${retailer} reviews`);
        }
