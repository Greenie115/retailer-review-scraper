...
function updateTescoHandler() {
  try {
    ...
    const tescoHandlerPath = path.join(__dirname, 'tesco-handler-button-improved.js');
    const tescoHandler = fs.readFileSync(tescoHandlerPath, 'utf8');
    
    // Extract the handleTescoSite function from the new file
    const functionRegex = /async function handleTescoSite\(page, siteConfig, maxReviews\) \{[\s\S]*?\n\}/;
    ...
    const backupPath = path.join(__dirname, 'review-scraper-crawlee-fixed.backup-tesco-button-improved.js');
